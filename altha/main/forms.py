...
class DoctorRegisterForm(ModelForm):
    ...
    specialty = forms.CharField(max_length=50,required=True)
    bio = forms.CharField(max_length=200,required=True)
    address = forms.CharField(max_length=50,required=True)
    landline_number = forms.CharField(max_length=17,required=True)
    emergency_line = forms.CharField(max_length=17,required=False)
...
class TakeAppointmentForm(ModelForm):
	...
	first_time_visit =  forms.BooleanField(required=False,initial=False,label='is this your first time visiting ?')
